...
bool control = true;
const char* scannerName="ClassRoom_B1-301";
...
void setup() {
  ...
  bool res;
  res = wm.autoConnect(scannerName, ""); // password protected ap
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    for (int i = 0; i < networksFound; i++) {
      ...
      if (control == true) {
        ...
        HTTPClient http;
        http.begin("http://10.0.0.5:8080/macAddress/scanphp.php?ssid=" + str + "&mac=" + macAdd + "&scanner=" + scannerName); // Specify the URL here
        http.GET();
        ...
        HTTPClient http;
        http.begin("http://10.0.0.5:8080/macAddress/check-mac.php?mac=" + macAdd); // Specify the URL here
        http.GET();
        String payload = http.getString();
        Serial.println(payload);
        http.end();
